...
function CDOTA_BaseNPC:GetDefaultBAT()
	local BAT = 0
	if self:IsHero() then
		-- DeepPrintTable(GameRules.HeroKV)
		BAT = GameRules.HeroKV[self:GetName()]['AttackRate'] or HeroKVBase[self:GetName()]['AttackRate']
	else
...
function CDOTA_BaseNPC:GetGibType()
	local BAT = 0
	if self:IsHero() then
		BAT = GameRules.HeroKV[self:GetName()]['GibType'] or HeroKVBase[self:GetName()]['GibType']
	else
...
function CDOTA_BaseNPC_Hero:GetHeroColor()
	local str = GameRules.HeroKV[self:GetName()]['HeroGlowColor'] or HeroKVBase[self:GetName()]['HeroGlowColor'] or GameRules.HeroKV[self:GetName()]['GibTintColor'] or HeroKVBase[self:GetName()]['GibTintColor']
	--print(str)
	if not str then
...
function GetHeroMainAttr(sHeroname)
	if GameRules.HeroKV[sHeroname] and GameRules.HeroKV[sHeroname]['AttributePrimary'] then
