...
LinkLuaModifier("modifier_imba_whirling_axes_melee_debuff_dummy", "ability/abilities_lua/hero_troll_warlord", LUA_MODIFIER_MOTION_NONE)
...
LinkLuaModifier("modifier_imba_whirling_axes_ranged_debuff", "ability/abilities_lua/hero_troll_warlord", LUA_MODIFIER_MOTION_NONE)

imba_troll_warlord_whirling_axes_ranged_d = class({})

function imba_troll_warlord_whirling_axes_ranged_d:IsHiddenWhenStolen() 		return false end
function imba_troll_warlord_whirling_axes_ranged_d:IsRefreshable() 			return true end
function imba_troll_warlord_whirling_axes_ranged_d:IsStealable() 				return true end
function imba_troll_warlord_whirling_axes_ranged_d:IsNetherWardStealable()	return true end
function imba_troll_warlord_whirling_axes_ranged_d:GetCastRange() return self:GetSpecialValueFor("range") end

function imba_troll_warlord_whirling_axes_ranged_d:OnSpellStart()
	local caster = self:GetCaster()
...
function imba_troll_warlord_whirling_axes_ranged_d:OnProjectileHit_ExtraData(target, location, keys)
	if target and not IsInTable(target, EntIndexToHScript(keys.thinker).hitted) then
		...
		ApplyDamage({attacker = self:GetCaster(), victim = target, damage = self:GetSpecialValueFor("damage")+self:GetCaster():GetAgility()*self:GetSpecialValueFor("attr_scale"), damage_type = self:GetAbilityDamageType(), damage_flags = DOTA_DAMAGE_FLAG_NONE, ability = self})
...
imba_troll_warlord_whirling_axes_ranged_c = class({})

function imba_troll_warlord_whirling_axes_ranged_c:IsHiddenWhenStolen() 		return false end
function imba_troll_warlord_whirling_axes_ranged_c:IsRefreshable() 			return true end
function imba_troll_warlord_whirling_axes_ranged_c:IsStealable() 				return true end
function imba_troll_warlord_whirling_axes_ranged_c:IsNetherWardStealable()	return true end
function imba_troll_warlord_whirling_axes_ranged_c:GetCastRange() return self:GetSpecialValueFor("range") end

function imba_troll_warlord_whirling_axes_ranged_c:OnSpellStart()
	local caster = self:GetCaster()
	caster:EmitSound("Hero_TrollWarlord.WhirlingAxes.Ranged")
	local direction = (self:GetCursorPosition() - caster:GetAbsOrigin()):Normalized()
	direction.z = 0
...
function imba_troll_warlord_whirling_axes_ranged_c:OnProjectileHit_ExtraData(target, location, keys)
	if target and not IsInTable(target, EntIndexToHScript(keys.thinker).hitted) then
		table.insert(EntIndexToHScript(keys.thinker).hitted, target)
		target:EmitSound("Hero_TrollWarlord.WhirlingAxes.Target")
		ApplyDamage({attacker = self:GetCaster(), victim = target, damage = self:GetSpecialValueFor("damage")+self:GetCaster():GetAgility()*self:GetSpecialValueFor("attr_scale"), damage_type = self:GetAbilityDamageType(), damage_flags = DOTA_DAMAGE_FLAG_NONE, ability = self})
...
imba_troll_warlord_whirling_axes_ranged_b = class({})

function imba_troll_warlord_whirling_axes_ranged_b:IsHiddenWhenStolen() 		return false end
function imba_troll_warlord_whirling_axes_ranged_b:IsRefreshable() 			return true end
function imba_troll_warlord_whirling_axes_ranged_b:IsStealable() 				return true end
function imba_troll_warlord_whirling_axes_ranged_b:IsNetherWardStealable()	return true end
function imba_troll_warlord_whirling_axes_ranged_b:GetCastRange() return self:GetSpecialValueFor("range") end

function imba_troll_warlord_whirling_axes_ranged_b:OnSpellStart()
	local caster = self:GetCaster()
	caster:EmitSound("Hero_TrollWarlord.WhirlingAxes.Ranged")
	local direction = (self:GetCursorPosition() - caster:GetAbsOrigin()):Normalized()
	direction.z = 0
	...
	local angle = angles / axes
	for i = 1, axes do
		local pos = RotatePosition(caster:GetAbsOrigin(), QAngle(0, (i-3) * angle, 0), endpos)
		local thinker = CreateModifierThinker(caster, self, "modifier_imba_whirling_axes_ranged_debuff", {}, caster:GetAbsOrigin(), caster:GetTeamNumber(), false):entindex()
		EntIndexToHScript(thinker).hitted = {}
		local info = 
		{
...
function imba_troll_warlord_whirling_axes_ranged_b:OnProjectileHit_ExtraData(target, location, keys)
	if target and not IsInTable(target, EntIndexToHScript(keys.thinker).hitted) then
		table.insert(EntIndexToHScript(keys.thinker).hitted, target)
		target:EmitSound("Hero_TrollWarlord.WhirlingAxes.Target")
		ApplyDamage({attacker = self:GetCaster(), victim = target, damage = self:GetSpecialValueFor("damage")+self:GetCaster():GetAgility()*self:GetSpecialValueFor("attr_scale"), damage_type = self:GetAbilityDamageType(), damage_flags = DOTA_DAMAGE_FLAG_NONE, ability = self})
...
imba_troll_warlord_whirling_axes_ranged_a = class({})

function imba_troll_warlord_whirling_axes_ranged_a:IsHiddenWhenStolen() 		return false end
function imba_troll_warlord_whirling_axes_ranged_a:IsRefreshable() 			return true end
function imba_troll_warlord_whirling_axes_ranged_a:IsStealable() 				return true end
function imba_troll_warlord_whirling_axes_ranged_a:IsNetherWardStealable()	return true end
function imba_troll_warlord_whirling_axes_ranged_a:GetCastRange() return self:GetSpecialValueFor("range") end

function imba_troll_warlord_whirling_axes_ranged_a:OnSpellStart()
	local caster = self:GetCaster()
	caster:EmitSound("Hero_TrollWarlord.WhirlingAxes.Ranged")
	local direction = (self:GetCursorPosition() - caster:GetAbsOrigin()):Normalized()
	direction.z = 0
...
function imba_troll_warlord_whirling_axes_ranged_a:OnProjectileHit_ExtraData(target, location, keys)
	if target and not IsInTable(target, EntIndexToHScript(keys.thinker).hitted) then
		table.insert(EntIndexToHScript(keys.thinker).hitted, target)
		target:EmitSound("Hero_TrollWarlord.WhirlingAxes.Target")
		ApplyDamage({attacker = self:GetCaster(), victim = target, damage = self:GetSpecialValueFor("damage")+self:GetCaster():GetAgility()*self:GetSpecialValueFor("attr_scale"), damage_type = self:GetAbilityDamageType(), damage_flags = DOTA_DAMAGE_FLAG_NONE, ability = self})
...
imba_troll_warlord_whirling_axes_ranged_s = class({})

function imba_troll_warlord_whirling_axes_ranged_s:IsHiddenWhenStolen() 		return false end
function imba_troll_warlord_whirling_axes_ranged_s:IsRefreshable() 			return true end
function imba_troll_warlord_whirling_axes_ranged_s:IsStealable() 				return true end
function imba_troll_warlord_whirling_axes_ranged_s:IsNetherWardStealable()	return true end
function imba_troll_warlord_whirling_axes_ranged_s:GetCastRange() return self:GetSpecialValueFor("range") end

function imba_troll_warlord_whirling_axes_ranged_s:OnSpellStart()
	local caster = self:GetCaster()
	caster:EmitSound("Hero_TrollWarlord.WhirlingAxes.Ranged")
	local direction = (self:GetCursorPosition() - caster:GetAbsOrigin()):Normalized()
	direction.z = 0
...
function imba_troll_warlord_whirling_axes_ranged_s:OnProjectileHit_ExtraData(target, location, keys)
	if target and not IsInTable(target, EntIndexToHScript(keys.thinker).hitted) then
		table.insert(EntIndexToHScript(keys.thinker).hitted, target)
		target:EmitSound("Hero_TrollWarlord.WhirlingAxes.Target")
		ApplyDamage({attacker = self:GetCaster(), victim = target, damage = self:GetSpecialValueFor("damage")+self:GetCaster():GetAgility()*self:GetSpecialValueFor("attr_scale"), damage_type = self:GetAbilityDamageType(), damage_flags = DOTA_DAMAGE_FLAG_NONE, ability = self})
...
function modifier_imba_whirling_axes_ranged_debuff:GetModifierMoveSpeedBonus_Percentage() return (0 - ((self:GetAbility():GetSpecialValueFor("base_slow") or 0) + self:GetStackCount() * (self:GetAbility():GetSpecialValueFor("stack_slow") or 0))) end


LinkLuaModifier("modifier_imba_fervor_passive", "ability/abilities_lua/hero_troll_warlord", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_fervor_dummy", "ability/abilities_lua/hero_troll_warlord", LUA_MODIFIER_MOTION_NONE)

imba_troll_warlord_fervor_a = class({})

function imba_troll_warlord_fervor_a:GetIntrinsicModifierName() return "modifier_imba_fervor_passive" end

imba_troll_warlord_fervor_s = class({})

function imba_troll_warlord_fervor_s:GetIntrinsicModifierName() return "modifier_imba_fervor_passive" end
...
function modifier_imba_fervor_passive:OnAttackLanded(keys)
	if IsServer() then
		if not self:GetParent():PassivesDisabled() and keys.attacker == self:GetParent() and not self:GetParent():HasModifier("modifier_imba_berserkers_rage_no_dmg") and not self:GetParent():IsIllusion() then
...
function modifier_imba_fervor_dummy:GetModifierAttackSpeedBonus_Constant() return (self:GetParent():IsIllusion() and 0 or (self:GetStackCount() * self:GetAbility():GetSpecialValueFor("bonus_as") or 0)) end

imba_troll_warlord_berserkers_rage = class({})

LinkLuaModifier("modifier_imba_berserkers_rage_passive", "ability/abilities_lua/hero_troll_warlord", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_berserkers_rage_no_dmg", "ability/abilities_lua/hero_troll_warlord", LUA_MODIFIER_MOTION_NONE)
...
LinkLuaModifier("modifier_imba_battle_trance", "ability/abilities_lua/hero_troll_warlord", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_battle_trance_stacks", "ability/abilities_lua/hero_troll_warlord", LUA_MODIFIER_MOTION_NONE)
