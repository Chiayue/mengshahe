...
LinkLuaModifier("modifier_imba_hoof_stomp_caster", "ability/abilities_lua/hero_centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_hoof_stomp_enemy", "ability/abilities_lua/hero_centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_hoof_stomp_dummy", "ability/abilities_lua/hero_centaur", LUA_MODIFIER_MOTION_NONE)

imba_centaur_hoof_stomp_d = class({})

function imba_centaur_hoof_stomp_d:IsHiddenWhenStolen() 		return false end
function imba_centaur_hoof_stomp_d:IsRefreshable() 			return true  end
function imba_centaur_hoof_stomp_d:IsStealable() 				return true  end
function imba_centaur_hoof_stomp_d:IsNetherWardStealable() 	return true end

function imba_centaur_hoof_stomp_d:GetCastRange(vLocation, hTarget) return self:GetSpecialValueFor("radius") - self:GetCaster():GetCastRangeBonus() end

function imba_centaur_hoof_stomp_d:OnSpellStart()
	...
	for _,enemy in pairs(enemies) do
		...
		local damageTable = {
							victim = enemy,
							attacker = caster,
							damage = self:GetSpecialValueFor("stomp_damage")+self:GetSpecialValueFor("attr_scale")*caster:GetStrength(),
...
imba_centaur_hoof_stomp_c = class({})

function imba_centaur_hoof_stomp_c:IsHiddenWhenStolen() 		return false end
function imba_centaur_hoof_stomp_c:IsRefreshable() 			return true  end
function imba_centaur_hoof_stomp_c:IsStealable() 				return true  end
function imba_centaur_hoof_stomp_c:IsNetherWardStealable() 	return true end

function imba_centaur_hoof_stomp_c:GetCastRange(vLocation, hTarget) return self:GetSpecialValueFor("radius") - self:GetCaster():GetCastRangeBonus() end

function imba_centaur_hoof_stomp_c:OnSpellStart()
	local caster = self:GetCaster()
	local enemies = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, self:GetSpecialValueFor("radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_DAMAGE_FLAG_NONE, FIND_ANY_ORDER, false)
	for _,enemy in pairs(enemies) do
		enemy:AddNewModifier(caster, self, "modifier_imba_stunned", {duration = self:GetSpecialValueFor("stun_duration")})
...
imba_centaur_hoof_stomp_b = class({})

function imba_centaur_hoof_stomp_b:IsHiddenWhenStolen() 		return false end
function imba_centaur_hoof_stomp_b:IsRefreshable() 			return true  end
function imba_centaur_hoof_stomp_b:IsStealable() 				return true  end
function imba_centaur_hoof_stomp_b:IsNetherWardStealable() 	return true end

function imba_centaur_hoof_stomp_b:GetCastRange(vLocation, hTarget) return self:GetSpecialValueFor("radius") - self:GetCaster():GetCastRangeBonus() end

function imba_centaur_hoof_stomp_b:OnSpellStart()
	local caster = self:GetCaster()
	local enemies = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, self:GetSpecialValueFor("radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_DAMAGE_FLAG_NONE, FIND_ANY_ORDER, false)
	for _,enemy in pairs(enemies) do
		enemy:AddNewModifier(caster, self, "modifier_imba_stunned", {duration = self:GetSpecialValueFor("stun_duration")})
...
imba_centaur_hoof_stomp_a = class({})

function imba_centaur_hoof_stomp_a:IsHiddenWhenStolen() 		return false end
function imba_centaur_hoof_stomp_a:IsRefreshable() 			return true  end
function imba_centaur_hoof_stomp_a:IsStealable() 				return true  end
function imba_centaur_hoof_stomp_a:IsNetherWardStealable() 	return true end

function imba_centaur_hoof_stomp_a:GetCastRange(vLocation, hTarget) return self:GetSpecialValueFor("radius") - self:GetCaster():GetCastRangeBonus() end

function imba_centaur_hoof_stomp_a:OnSpellStart()
	local caster = self:GetCaster()
	local enemies = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, self:GetSpecialValueFor("radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_DAMAGE_FLAG_NONE, FIND_ANY_ORDER, false)
	for _,enemy in pairs(enemies) do
		enemy:AddNewModifier(caster, self, "modifier_imba_stunned", {duration = self:GetSpecialValueFor("stun_duration")})
...
imba_centaur_hoof_stomp_s = class({})

function imba_centaur_hoof_stomp_s:IsHiddenWhenStolen() 		return false end
function imba_centaur_hoof_stomp_s:IsRefreshable() 			return true  end
function imba_centaur_hoof_stomp_s:IsStealable() 				return true  end
function imba_centaur_hoof_stomp_s:IsNetherWardStealable() 	return true end

function imba_centaur_hoof_stomp_s:GetCastRange(vLocation, hTarget) return self:GetSpecialValueFor("radius") - self:GetCaster():GetCastRangeBonus() end

function imba_centaur_hoof_stomp_s:OnSpellStart()
	local caster = self:GetCaster()
	local enemies = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, self:GetSpecialValueFor("radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_DAMAGE_FLAG_NONE, FIND_ANY_ORDER, false)
	for _,enemy in pairs(enemies) do
		enemy:AddNewModifier(caster, self, "modifier_imba_stunned", {duration = self:GetSpecialValueFor("stun_duration")})
...
function modifier_imba_hoof_stomp_dummy:GetAuraRadius() return (self:GetAbility():GetSpecialValueFor("radius") or 0) end
...
function modifier_imba_hoof_stomp_dummy:OnIntervalThink()
	local enemies = FindUnitsInRadius(self:GetParent():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil, (self:GetAbility():GetSpecialValueFor("radius") or 0), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
	for _,enemy in pairs(enemies) do
...
LinkLuaModifier("modifier_imba_centaur_return_aura", "ability/abilities_lua/hero_centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_centaur_return_active", "ability/abilities_lua/hero_centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_centaur_return", "ability/abilities_lua/hero_centaur", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_centaur_return_prevent", "ability/abilities_lua/hero_centaur", LUA_MODIFIER_MOTION_NONE)
...
LinkLuaModifier("modifier_imba_centaur_stampede", "ability/abilities_lua/hero_centaur", LUA_MODIFIER_MOTION_NONE)
