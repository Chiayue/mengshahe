...
function utils_popups:ShowHealing(target, amount)
...
function utils_popups:ShowDamage(target, amount)
    PopupNumbers(target, "damage", Vector(255, 0, 0), 1.0, amount, nil, POPUP_SYMBOL_POST_DROP)
end

-- e.g. when dealing critical damage
function utils_popups:ShowCriticalDamage(target, amount)
    PopupNumbers(target, "crit", Vector(255, 0, 0), 1.0, amount, nil, POPUP_SYMBOL_POST_LIGHTNING)
end

function utils_popups:ShowCriticalDamage2(target, amount)
    PopupNumbers(target, "crit2", Vector(255, 0, 0), 1.0, amount, nil, POPUP_SYMBOL_POST_LIGHTNING)
end

-- e.g. when taking damage over time from a poison type spell
function utils_popups:ShowDamageOverTime(target, amount)
